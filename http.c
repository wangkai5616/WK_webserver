...
static void serve_static(int fd, char *filename, size_t filesize, wk_http_out_t *out);

static char *ROOT = NULL;

mime_type_t wkeed_mime[] = 
{
    {".html", "text/html"},
    {".xml", "text/xml"},
...
//获取文件类型,比如根据html得到text/html
const char* get_file_type(const char *type){
    // 将type和索引表中后缀比较，返回类型用于填充Content-Type字段
    for(int i = 0; wkeed_mime[i].type != NULL; ++i){
        if(strcmp(type, wkeed_mime[i].type) == 0)
            return wkeed_mime[i].value;
    }
    // 未识别返回"text/plain"
    return "text/plain";
}
...
void do_error(int fd, char *cause, char *err_num, char *short_msg, char *long_msg){
    ...
    sprintf(body, "<html><title>wkeed Error<title>");
    sprintf(body, "%s<body bgcolor=""ffffff"">\n", body);
    sprintf(body, "%s%s : %s\n", body, err_num, short_msg);
    sprintf(body, "%s<p>%s : %s\n</p>", body, long_msg, cause);
    sprintf(body, "%s<hr><em>wkeed web server</em>\n</body></html>", body);

    // 返回错误码，组织错误响应头
    sprintf(header, "HTTP/1.1 %s %s\r\n", err_num, short_msg);
    sprintf(header, "%sServer: wkeed\r\n", header);
...
// 实际就是给对端的应答
// 处理静态文件请求
void serve_static(int fd, char *filename, size_t filesize, wk_http_out_t *out){
    // 响应头缓冲（512字节）和数据缓冲（8192字节）
    char header[MAXLINE];
    char buff[SHORTLINE];//保存的是最后修改的时间
    struct tm tm;
...
int error_proess(struct stat* sbufptr, char *filename, int fd){
    ...
    if(stat(filename, sbufptr) < 0){
        do_error(fd, filename, "404", "Not Found", "wkeed can't find the file");
        return 1;
    }

    // 处理权限错误
    if(!(S_ISREG((*sbufptr).st_mode)) || !(S_IRUSR & (*sbufptr).st_mode)){
        do_error(fd, filename, "403", "Forbidden", "wkeed can't read the file");
        return 1;
    }
...
void do_request(void* ptr){
    wk_http_request_t* request = (wk_http_request_t*)ptr;
    int fd = request->fd;//已连接套接字fd产生信息
    ROOT = request->root;
    ...
    while(1){
        ...
        // 解析请求报文行
        rc = wk_http_parse_request_line(request);
        if(rc == wk_AGAIN)
            continue;
        else if(rc != 0)
            goto err;
        ...
        // 解析请求报文体
        rc = wk_http_parse_request_body(request);
        if(rc == wk_AGAIN)
            continue;
        else if(rc != 0)
            goto err;
        ...
        // 分配并初始化返回数据结构
        wk_http_out_t* out = (wk_http_out_t *)malloc(sizeof(wk_http_out_t));
        wk_init_out_t(out, fd);
		...
		//根据解析出来的请求头进行处理
        wk_http_handle_header(request, out);
    ...
    // 修改已经注册描述符的事件类型
    // 重置定时器，每等待下一次请求均生效
    wk_epoll_mod(request->epoll_fd, request->fd, request, (EPOLLIN | EPOLLET | EPOLLONESHOT));
    wk_add_timer(request, TIMEOUT_DEFAULT, wk_http_close_conn);
