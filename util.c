...
//读取配置文件filename中的内容，保存到结构体conf中
int read_conf(char* filename, wk_conf_t* conf){
    // 以只读方式打开文件
    FILE* fp = fopen(filename, "r");
    if(!fp)
        return wk_CONF_ERROR;
	...
	从文件指针stream中读取n-1个字符，存到以str为起始地址的空间里，
	直到读完一行，如果成功则返回str的指针，否则返回NULL。
	*/
...
void accept_connection(int listen_fd, int epoll_fd, char* path){
    ...
    // 申请wk_http_request_t类型节点并初始化
    wk_http_request_t* request = (wk_http_request_t*)malloc(sizeof(wk_http_request_t));
    wk_init_request_t(request, accept_fd, epoll_fd, path);

    // 文件描述符可以读，边缘触发(Edge Triggered)模式，保证一个socket连接在任一时刻只被一个线程处理
    //EPOLLONESHOT：只监听一次事件，当监听完这次事件之后，如果还需要继续监听这个socket的话，
    //需要再次把这个socket加入到EPOLL队列里
    wk_epoll_add(epoll_fd, accept_fd, request, (EPOLLIN | EPOLLET | EPOLLONESHOT));
